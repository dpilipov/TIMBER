...
def CompareShapes(outfilename,year,prettyvarname,bkgs={},signals={},names={},colors={},
    ...
    if doSoverB:
        ...
        SoverB.cd()
#DP EDIT for Punzi significance
        s_over_b.GetYaxis().SetTitle('S/(n/2+#sqrt{B})')
...
#DP EDIT change from sqrt(B) to n/2 + sqrt(b), with n=2
    nuse = 2
    total_bkgs = stack_of_bkgs.GetStack().Last()
    total_bkgs.Add(signal)
...
#DP EDIT - no longer need to check for negative background!
#           val = sig_int.GetBinContent(ix)/(nuse/2.0 + math.sqrt(bkg_int.GetBinContent(ix)))
